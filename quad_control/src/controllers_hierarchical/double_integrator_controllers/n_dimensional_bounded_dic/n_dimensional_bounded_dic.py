...
class NDimensionalBoundedDIC(dic.DoubleIntegratorController): 
    ...
    def  _DI_Bounded(self,p,v):
        ...
        if pp >= eps:
            ...
            for i in range(size_di):
                ee = I[:,i];
                h1_p_p[:,:,i] = kp*D2sat_p/sigma_p**2*numpy.outer(p,p)/(pp**2)*dot(p,ee)       + \
                                kp*Dsat_p/sigma_p*dot(p,ee)/pp*I                         + \
                                kp*Dsat_p/sigma_p*numpy.outer(p,ee)/pp                         + \
                                (-1)*kp*Dsat_p/sigma_p*numpy.outer(p,p)/pp**3*dot(p,ee)        + \
        ...
        if vv >= eps: 
            ...
            for i in range(size_di):
                ee = I[:,i]
                h2_v_v[:,:,i] = kv*D2sat_v/sigma_v**2*numpy.outer(v,v)/(vv**2)*dot(v,ee)    + \
                                kv*Dsat_v/sigma_v*dot(v,ee)/vv*I                      + \
                                kv*Dsat_v/sigma_v*numpy.outer(v,ee)/vv                      + \
                                (-1)*kv*Dsat_v/sigma_v*numpy.outer(v,v)/(vv**3)*dot(v,ee)   + \
